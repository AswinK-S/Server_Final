...
//--------------------------------------------------------------------------------------------------------------Treatment
    ...
    //edit treatment name
    editTnameUseCase(name:string,id:string,next:Next):Promise<ITreatment|void>

//-----------------------------------------------------------------------------------Doctors

    //add Doctor
    addDoctor({name,email,mob,password,address,experience,doctor_id,treatment,subTreatment,amount}:IDoctor,next:Next):Promise <{doctor:IDoctor;message?:string;}| void>
    //getDoctor
...
//--------------------------------------------------------------------------------------users    
    ...
    //get users
    getUsersUsecase(page:number,next:Next):Promise<{users:Iuser[],totalUsers:number} |void>
//------dash data
 getDashDataUseCase(req:Req,next:Next):Promise<object|void>
